...
def parseOrbits(s: String): List[(String, String)] =
  s.trim
    .split("\n").flatMap(_.split(","))
...
```scala mdoc
sealed trait Tree
final case class Node(id: String, edges: Seq[Tree]) extends Tree
...
val orbitMap = Tree.fromList(parseOrbits(exampleInput), "COM")
```
...
def countOrbits(tree: Tree, depth: Int = 0): Int = tree match {
  case Node(_, edges) =>
    ...
    edges.foldLeft(0) { case (count, edge) =>
      count + newDepth + countEdges(edge, newDepth)
    }
  case Leaf(id) => 0
}
...
```scala mdoc
countOrbits(Tree.fromList(parseOrbits(input6), "COM"))
```
